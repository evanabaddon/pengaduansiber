...
use App\Filament\Bagbinopsnal\Resources\LaporanPolisiResource\Pages;
use App\Filament\Bagbinopsnal\Resources\LaporanPolisiResource\RelationManagers;
...
class LaporanPolisiResource extends Resource
{
                    ...
                    ->steps([
                        Wizard\Step::make('Pelapor')
                            ...
                            ->afterValidation(function ($state, $component) {
                                ...
                                if ($component->getLivewire() instanceof Pages\CreateLaporanPolisi) {
                        ...
                        Wizard\Step::make('Terlapor')
                            ...
                            ->afterValidation(function ($state, $component) {
                                // cek apakah sedang di halaman create  
                                if (request()->routeIs('filament.resources.laporan-polisis.create')) {
                            ...
                            ->schema([
                                Repeater::make('barangBuktis')
                                    ...
                                    ->mutateRelationshipDataBeforeCreateUsing(function (array $data): array {
                                        $data['buktiable_type'] = LaporanPolisi::class;
                                        return $data;
                                    }),
                            ]),
                        // wizard media
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('pelapors.nama')
                    ->searchable()
                    ->description(fn (LaporanPolisi $record): string => $record->korbans()->where('laporan_polisi_id', $record->id)->pluck('nama')->join(', '))
                ...
                ColumnGroup::make('YANG MENANGANI')
                    ...
                    ->columns([
                        SelectColumn::make('subdit_id')
                            ...
                            ->afterStateUpdated(function (LaporanPolisi $record, $state) {
                                $record->update([
                                    'unit_id' => null,
                                ]);
                        ...
                        SelectColumn::make('unit_id')
                            ...
                            ->options(function (LaporanPolisi $record) {
            ...
            ->actions([
                ActionGroup::make([
                    Action::make('Analyze')
                        ->visible(fn (LaporanPolisi $record): bool => env('AI_ANALYZER', false))
                        ->action(function (LaporanPolisi $record, OllamaService $ollamaService, DeepSeekService $deepSeekService) {
                            try {
                                ...
                                if (isset($analysis['data'])) {
                                    ...
                                    if (is_array($data) && 
                                        ...
                                        try {
                                            ...
                                            return redirect()->to(LaporanPolisiResource::getUrl('view-analysis', ['record' => $record]));
                    ...
                    Action::make('viewAnalysis')
                        ...
                        ->url(fn (LaporanPolisi $record): string => static::getUrl('view-analysis', ['record' => $record]))
                        ->visible(fn (LaporanPolisi $record): bool => $record->analysis()->exists()),
                ]),
                ActionsViewAction::make()
                    ->label(false)
                    ->modalContent(fn (LaporanPolisi $record): View => view(
                        'filament.resources.laporan-polisi.view',
                        ['record' => $record]
                    ))
                    ->modalWidth('7xl'), 
    ...
    public static function getPages(): array
    {
        return [
            ...
            'view' => Pages\ViewLaporanPolisi::route('/{record}'),
