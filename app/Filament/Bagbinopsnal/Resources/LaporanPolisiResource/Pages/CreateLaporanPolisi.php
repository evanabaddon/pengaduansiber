...
class CreateLaporanPolisi extends CreateRecord
{
    ...
    protected function loadExistingDraft(): void
    {
        try {
            $draft = FormDraft::where('user_id', auth()->id())
                ->where('form_type', 'laporan_polisi')
                ->first();
    ...
    public function handleRecordCreation(array $data): LaporanPolisi
    {
        ...
        try {
            ...
            $laporanPolisi = LaporanPolisi::create($data);
            ...
            if (isset($data['barangBuktis'])) {
                $laporanPolisi->barangBuktis()->createMany($data['barangBuktis']);
            }
