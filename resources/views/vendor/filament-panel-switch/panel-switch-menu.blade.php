@if ($isSimple)
    <x-filament::dropdown teleport placement="bottom-end">
        <x-slot name="trigger">
            ...
            <button type="button"
                class="fi-sidebar-group-button flex items-center gap-x-3 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                <span class="w-5 h-5 font-semibold bg-white rounded-full shrink-0 text-primary-500">
                    {{ str($labels[$currentPanel->getId()] ?? $currentPanel->getId())->substr(0,1)->upper() }}
                </span>
                <span>{{ $labels[$currentPanel->getId()] ?? str($currentPanel->getId())->ucfirst() }}</span>
        ...
        <x-filament::dropdown.list>
            ...
            <x-filament::dropdown.list.item
                :href="$url"
                tag="a"
                class="fi-sidebar-group-item flex items-center gap-x-3 px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"
            >
                {{ $labels[$id] ?? str($id)->ucfirst() }}
            </x-filament::dropdown.list.item>
            @endforeach
